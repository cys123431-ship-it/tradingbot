...
def test_tp2_order_type_can_override_to_take_profit_market(monkeypatch):
    async def run():
        cls = emas.MainController
        bot = cls.__new__(cls)
        ...
        bot.safe_amount = lambda symbol, qty: qty
        bot.safe_price = lambda symbol, price: price
        bot._build_protection_client_order_id = lambda *a, **k: "utbtp2BTCUSDTcid"
        bot._create_protection_order_with_retries = fake_create
        ...
        await bot._place_reduce_only_tp_from_plan(
            plan,
            tp,
            {"tp_order_type": "LIMIT", "tp2_order_type": "TAKE_PROFIT_MARKET"},
        )
