...
def test_utbreakout_watchdog_sends_full_report_after_ready_without_order():
    notifications = []
    ...
    joined = "\n".join(notifications)
    assert "UTBreakout Watchdog" in joined
    assert "READY_BUT_NO_ORDER_ATTEMPT" in joined
    assert "UTBreakout Entry Trace Report" in joined
