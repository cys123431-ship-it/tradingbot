...
- 한 화면에 바이낸스 선물과 European Options 프리미엄 1분봉 차트를 동시 표시
- 실제 거래소 포지션(봇 진입 및 수동 진입 모두)
- 진입가, 현재가, SL, TP 주문 라인
- 증거금, 레버리지, 미실현 PnL, ROE, 청산가
- 옵션 수량, 진입 비용, 현재 가치, 프리미엄 손익, DTE, IV, Greeks
- 봇 옵션에만 소프트웨어 손절·최종목표·활성 추적청산선을 구분해 표시
...
- 옵션 API 키도 Windows로 복사하지 않으며, 서버에서 제한된 주기로 실제 포지션과 대조한 결과만 전송합니다.
