...
Telegram 전용 메뉴는 `/ema200`이며 메인 키보드에도 버튼이 있습니다. 전략 선택, 신규진입 ON/OFF, 1회 위험%, 레버리지, 비상 손절 가격거리, 일 손실 한도, 최근 7일 손실 한도를 버튼 또는 직접입력으로 조정할 수 있습니다. 각 직접입력 버튼은 일반 사용자가 이해할 수 있도록 계산 예시와 적용 범위를 먼저 표시합니다. 이 전략의 UT Bot은 `Key=1.0`, `ATR=10`, `HA=OFF` 전용값을 사용하므로 별도 `/utbot` 전략의 설정 변경에 영향을 받지 않습니다.
