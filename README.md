...
이 전략은 기존 UTRSI 계열을 이름만 바꾼 것이 아니라 **별도 독립 전략**으로 구현됩니다. 실행 시간프레임은 완료된 `2h` 봉으로 고정합니다.
...
- `최적 후보 선택`이 ON이면 같은 완료 2시간봉에서 고정 10개를 모두 평가한 뒤 유효 신호만 비교합니다. 최근 UT 신호, RSI 진행 강도, 진입 방향 EMA200 기울기, 최근 24시간 거래대금을 가점으로 사용하고 EMA200에서 3ATR보다 멀어진 후보에는 추격 감점을 적용해 점수순으로 한 종목만 주문합니다. 이 점수는 이미 유효한 후보의 순서만 정하며 원래 EMA/UT/RSI 진입 조건을 우회하지 않습니다. OFF이면 기존 회전 스캔 순서의 첫 유효 신호부터 주문합니다.

정상 청산 규칙:

- LONG 보유 중 완료된 2시간봉에서 새로운 UT Bot Sell 신호가 발생하면 즉시 정상 청산합니다.
- SHORT 보유 중 완료된 2시간봉에서 새로운 UT Bot Buy 신호가 발생하면 즉시 정상 청산합니다.
- 정상 청산에는 R2/CHOP/Kalman 같은 선택적 청산 필터를 적용하지 않습니다.

리스크와 비상 보호는 정상 청산과 분리합니다.

- 계좌 equity가 **1,000 USDT 이하**이면 연속손실 단계형 소액계좌 규칙을 사용합니다. 첫 단계는 equity의 50%를 증거금으로 5x 진입하고 거래소 Stop 없이 UT 반대 신호로만 청산합니다.
...
- 고정 익절은 두지 않습니다. 소액계좌 첫 단계를 제외한 진입에는 비상탈출 Stop만 거래소 보호 주문으로 설치합니다.
...
Telegram 전용 메뉴는 `/ema200`이며 메인 키보드에도 버튼이 있습니다. 전략 선택, 신규진입 ON/OFF, 최적 후보 선택 ON/OFF, 1회 위험%, 레버리지, 비상 손절 가격거리, 일 손실 한도, 최근 7일 손실 한도를 버튼 또는 직접입력으로 조정할 수 있습니다. 상태 화면에는 최근 후보 순위와 실제 선택 종목도 표시합니다. 각 직접입력 버튼은 일반 사용자가 이해할 수 있도록 계산 예시와 적용 범위를 먼저 표시합니다. 이 전략의 UT Bot은 `Key=1.0`, `ATR=10`, `HA=OFF` 전용값을 사용하므로 별도 `/utbot` 전략의 설정 변경에 영향을 받지 않습니다.
