...
- LONG: 완료봉 종가가 EMA200 위 → UT Bot Buy가 이미 발생하고 LONG 상태가 유지됨 → 그 상태에서 현재 완료봉 RSI가 50을 아래에서 위로 새로 돌파할 때 진입합니다.
- SHORT: 완료봉 종가가 EMA200 아래 → UT Bot Sell이 이미 발생하고 SHORT 상태가 유지됨 → 그 상태에서 현재 완료봉 RSI가 50을 위에서 아래로 새로 돌파할 때 진입합니다.
- RSI가 먼저 50을 돌파하고 나중에 UT 신호가 나온 경우는 인정하지 않습니다. 과거 RSI 돌파를 저장해 뒤늦게 결합하지 않습니다.
- UT 신호와 RSI 돌파가 같은 완료봉에 함께 나타난 경우에도 봉 내부의 선후관계를 증명할 수 없으므로 진입하지 않습니다.
