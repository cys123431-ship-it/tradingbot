...
- 포지션 크기는 계좌 equity의 `risk_per_trade_percent`와 비상탈출 거리로 역산합니다.
- 기본값은 1회 위험 0.50%, 레버리지 5x, 비상탈출 5%, 일일 실현손실 한도 2%, 최근 7일 실현손실 한도 5%입니다.
- 일/7일 손실 한도에 도달해도 보유 포지션을 강제청산하지 않고 **신규진입만 차단**합니다.
- 고정 익절은 두지 않습니다. 비상탈출 Stop만 거래소 보호 주문으로 설치하며 완전히 OFF할 수 없습니다.
...
직접입력 안전 범위는 1회 위험 `0.10~5.00%`, 레버리지 `1~10x`, 비상탈출 `0.5~30%`, 일일 손실 `0.5~20%`, 최근 7일 손실 `1~40%`입니다. 최근 7일 한도는 일일 한도보다 작아지지 않도록 정규화하며, 비상 Stop·청산가 검사·단일 포지션·Critical Pause·중복주문 방지·보호주문 감사·owner 인증은 메뉴에서 끌 수 없습니다.
