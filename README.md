...
- 계좌 equity가 **1,000 USDT를 초과**하면 기존처럼 `risk_per_trade_percent`와 비상탈출 거리로 포지션 크기를 역산합니다.
...
- 비상탈출/위험 설정 변경은 새 포지션부터 적용하며, 기존 포지션의 보호 주문을 설정 변경만으로 자동 교체하지 않습니다.

Telegram 전용 메뉴는 `/ema200`이며 메인 키보드에도 버튼이 있습니다. 전략 선택, 신규진입 ON/OFF, 1회 위험%, 레버리지, 비상탈출 거리, 일 손실 한도, 최근 7일 손실 한도를 버튼 또는 직접입력으로 조정할 수 있습니다. 각 직접입력 버튼은 일반 사용자가 이해할 수 있도록 계산 예시와 적용 범위를 먼저 표시합니다.
