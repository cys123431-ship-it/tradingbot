...
## EMA200 + UT Bot + RSI (2H) 독립 전략

이 전략은 기존 UTRSI 계열을 이름만 바꾼 것이 아니라 **별도 독립 전략**으로 구현됩니다. 진입은 완료된 `2h` 봉으로 고정하며, 청산은 `/ema200`에서 `15m`(기본)·`30m`·`1h` 완료봉 중 선택합니다.

진입 규칙:

- LONG: 완료봉 종가가 EMA200 위 → UT Bot Buy가 먼저 발생하고 LONG 상태가 유지됨 → 현재 완료봉 RSI가 50보다 높고 직전 완료봉보다 상승할 때 진입합니다.
- SHORT: 완료봉 종가가 EMA200 아래 → UT Bot Sell이 먼저 발생하고 SHORT 상태가 유지됨 → 현재 완료봉 RSI가 50보다 낮고 직전 완료봉보다 하락할 때 진입합니다.
- UT 신호는 현재 RSI 평가봉보다 먼저 확정되어야 합니다. 같은 완료봉에서 UT와 RSI 조건이 함께 성립하면 봉 내부 선후관계를 증명할 수 없으므로 진입하지 않습니다.
- RSI 돌파를 저장하는 latch는 사용하지 않으며, 매 완료봉의 현재 RSI 수준과 방향을 새로 평가합니다.
- EMA 조건은 **완료봉 종가와 EMA200의 비교**입니다.
- 스캔 대상은 외부 시총 API를 호출하지 않는 고정 목록 `BTC, ETH, BNB, XRP, SOL, TRX, ZEC, HYPE, DOGE, XMR`입니다. 바이낸스가 시총 순위 API를 제공하지 않아, 2026-09-19 기준 비스테이블 시총 순위 중 바이낸스 USDT 무기한 선물 거래가 가능한 상위 10개를 고정했습니다. 다른 종목은 진입 직전에도 차단합니다.
- `최적 후보 선택`이 ON이면 같은 완료 2시간봉에서 고정 10개를 모두 평가한 뒤 유효 신호만 비교합니다. 최근 UT 신호, RSI 진행 강도, 진입 방향 EMA200 기울기, 최근 24시간 거래대금과 완료된 `15m`·`30m`·`1h` UT 상태를 보조 점수(방향 일치/반대 각각 ±4/±6/±8점, 조회 실패 0점)로 사용합니다. EMA200에서 3ATR보다 멀어진 후보에는 추격 감점을 적용합니다. 점수는 후보의 순서만 정하고 필수 진입 조건을 바꾸지 않습니다. OFF이면 기존 회전 스캔 순서의 첫 유효 신호부터 주문합니다.

정상 청산 규칙:

- LONG 보유 중 선택한 시간봉의 완료봉에서 새로운 UT Bot Sell 신호가 발생하면 정상 청산합니다.
- SHORT 보유 중 선택한 시간봉의 완료봉에서 새로운 UT Bot Buy 신호가 발생하면 정상 청산합니다.
- 정상 청산에는 R2/CHOP/Kalman 같은 선택적 청산 필터를 적용하지 않습니다.
- 증거금 기준 미실현 수익률이 **5%를 초과**하면 +5% 수익선에 거래소 Stop을 설치합니다. 10%, 15% 등을 초과하면 각각 +10%, +15% 수익선으로만 올립니다. 증거금 50 USDT·5x에서 수익률 6%라면 약 +2.5 USDT 손익선(롱 가격 +1%)에 Stop을 둡니다. 수수료·슬리피지 때문에 실제 체결 수익은 낮을 수 있으며, 선택한 UT 청산과 Stop 중 먼저 실행된 것이 포지션을 종료합니다.

리스크와 비상 보호는 정상 청산과 분리합니다.

- 계좌 equity가 **1,000 USDT 이하**이면 연속손실 단계형 소액계좌 규칙을 사용합니다. 첫 단계는 equity의 50%를 증거금으로 5x 진입하고 **초기 손실 Stop 없이** 선택한 UT 반대 신호를 따릅니다. 수익률 5% 초과 시에는 수익 보호 Stop을 추가합니다.
- 첫 손실 뒤 다음 진입부터 증거금 비율을 `35% → 25% → 15% → 10%(하한)`로 줄이며 비상탈출 Stop을 설치합니다. EMA200 전략 거래가 수익 또는 본전으로 끝나면 다음 진입은 50% 첫 단계로 초기화합니다. 손실 단계는 DB에 기록된 해당 전략의 실현손익으로 복구하므로 재시작해도 유지됩니다.
- 계좌 equity가 **1,000 USDT를 초과**하면 기존처럼 `risk_per_trade_percent`와 진입가 대비 비상 손절 가격거리로 포지션 크기를 역산합니다. Telegram 상태 화면은 현재 잔고를 기준으로 예상 명목 포지션, 필요 증거금, 각각의 계좌 대비 비율과 허용 손실예산을 함께 표시합니다. 예를 들어 equity 5,000 USDT, 위험 0.50%, 5x에서 손절거리 5%면 명목 약 500 USDT·증거금 약 100 USDT(계좌의 2%)이고, 손절거리 25%면 같은 25 USDT 손실예산을 지키기 위해 명목 약 100 USDT·증거금 약 20 USDT(계좌의 0.4%)로 줄어듭니다.
- 초과계좌 기본값은 1회 위험 0.50%, 레버리지 5x이며 공통 기본값은 비상탈출 5%, 일일 실현손실 한도 2%, 최근 7일 실현손실 한도 5%입니다.
- 일/7일 손실 한도에 도달해도 보유 포지션을 강제청산하지 않고 **신규진입만 차단**합니다.
- 고정 익절은 두지 않습니다. 소액계좌 첫 단계를 제외한 진입에는 비상탈출 Stop을 설치하고, 수익 기준 초과 시 거래소 Stop을 수익선으로 올립니다.
- 최소 주문금액 때문에 계획 위험을 초과하도록 주문수량을 자동 증액하지 않습니다.
- 비상 손절 가격거리/위험 설정 변경은 새 포지션부터 적용하며, 기존 포지션의 보호 주문을 설정 변경만으로 자동 교체하지 않습니다. 너무 먼 손절거리는 레버리지 청산가보다 늦어질 수 있으므로 기존 liquidation 안전검사가 진입을 차단합니다.

Telegram 전용 메뉴는 `/ema200`이며 메인 키보드에도 버튼이 있습니다. 전략 선택, 신규진입 ON/OFF, 최적 후보 선택 ON/OFF, UT 청산 시간봉 `15m/30m/1h`, 1회 위험%, 레버리지, 비상 손절 가격거리, 일 손실 한도, 최근 7일 손실 한도를 조정할 수 있습니다. 청산봉 변경은 현재 보유 포지션에도 다음 평가부터 적용됩니다. 상태 화면에는 최근 후보 순위와 실제 선택 종목도 표시합니다. 각 직접입력 버튼은 계산 예시와 적용 범위를 먼저 표시합니다. 이 전략의 UT Bot은 `Key=1.0`, `ATR=10`, `HA=OFF` 전용값을 사용합니다.

직접입력 안전 범위는 1회 위험 `0.10~5.00%`, 레버리지 `1~10x`, 비상탈출 `0.5~30%`, 일일 손실 `0.5~20%`, 최근 7일 손실 `1~40%`입니다. 위험%와 레버리지 입력은 1,000 USDT 초과 계좌에 적용되고, 소액계좌는 고정 5x 단계형 비율이 우선합니다. 최근 7일 한도는 일일 한도보다 작아지지 않도록 정규화합니다. 청산가 확인·단일 포지션·Critical Pause·중복주문 방지·owner 인증은 그대로 유지하며, 소액계좌 첫 단계의 의도적인 무Stop 상태는 DB 주문 상태에 명시적으로 기록해 일반적인 보호주문 누락과 구분합니다.
...
- **동적 레버리지**: 자동전략의 최종 진입계획에서 변동성·손절 거리·호가 품질·전략 합의·최근 전략 품질을 함께 평가해 2~10배를 선택합니다. 강한 단기 기회는 6~10배까지 허용하되 손절 위험예산은 별도로 유지하고, 8배 이상은 시간청산과 트레일링을 앞당깁니다.
